...
func (_q *ProjectMemberQuery) QueryInviter() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *ProjectMemberQuery) WithInviter(opts ...func(*UserQuery)) *ProjectMemberQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectMemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ProjectMember, error) {
	...
	if query := _q.withInviter; query != nil {
		if err := _q.loadInviter(ctx, query, nodes, nil,
			func(n *ProjectMember, e *User) { n.Edges.Inviter = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProjectMemberQuery) loadInviter(ctx context.Context, query *UserQuery, nodes []*ProjectMember, init func(*ProjectMember), assign func(*ProjectMember, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*ProjectMember)
	for i := range nodes {
...
func (_q *ProjectMemberQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withInviter != nil {
			_spec.Node.AddColumnOnce(projectmember.FieldInvitedBy)
		}
	}
...
func (_q *ProjectMemberQuery) Modify(modifiers ...func(s *sql.Selector)) *ProjectMemberSelect {
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *ProjectMemberSelect) Modify(modifiers ...func(s *sql.Selector)) *ProjectMemberSelect {
