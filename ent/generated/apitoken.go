...
type ApiToken struct {
	...
	Status apitoken.Status `json:"status,omitempty"`
...
func (*ApiToken) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case apitoken.FieldScopes, apitoken.FieldIPWhitelist:
			values[i] = new([]byte)
		case apitoken.FieldName, apitoken.FieldTokenHash, apitoken.FieldTokenPrefix, apitoken.FieldStatus:
			values[i] = new(sql.NullString)
		case apitoken.FieldExpiresAt, apitoken.FieldLastUsedAt, apitoken.FieldCreatedAt:
			values[i] = new(sql.NullTime)
		case apitoken.FieldID, apitoken.FieldUserID, apitoken.FieldProjectID:
			values[i] = new(uuid.UUID)
		case apitoken.ForeignKeys[0]: // project_api_tokens
	...
	for i := range columns {
		...
		case apitoken.FieldStatus:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.Status = apitoken.Status(value.String)
			}
		...
		case apitoken.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field project_api_tokens", values[i])
			} else if value.Valid {
				_m.project_api_tokens = new(uuid.UUID)
				*_m.project_api_tokens = *value.S.(*uuid.UUID)
			}
		default:
