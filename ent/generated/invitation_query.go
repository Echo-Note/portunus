...
func (_q *InvitationQuery) QueryProject() *ProjectQuery {
...
// QueryAcceptor chains the current query on the "acceptor" edge.
func (_q *InvitationQuery) QueryAcceptor() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *InvitationQuery) Clone() *InvitationQuery {
	...
	return &InvitationQuery{
		...
		withAcceptor: _q.withAcceptor.Clone(),
...
func (_q *InvitationQuery) WithProject(opts ...func(*ProjectQuery)) *InvitationQuery {
	query := (&ProjectClient{config: _q.config}).Query()
...
// WithAcceptor tells the query-builder to eager-load the nodes that are connected to
// the "acceptor" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *InvitationQuery) WithAcceptor(opts ...func(*UserQuery)) *InvitationQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAcceptor = query
	return _q
}
...
//		GroupBy(invitation.FieldProjectID).
...
//
//	client.Invitation.Query().
//		Select(invitation.FieldProjectID).
//		Scan(ctx, &v)
...
func (_q *InvitationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Invitation, error) {
	...
	if query := _q.withProject; query != nil {
		if err := _q.loadProject(ctx, query, nodes, nil,
			func(n *Invitation, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withAcceptor; query != nil {
		if err := _q.loadAcceptor(ctx, query, nodes, nil,
			func(n *Invitation, e *User) { n.Edges.Acceptor = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *InvitationQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *Project)) error {
...
func (_q *InvitationQuery) loadAcceptor(ctx context.Context, query *UserQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Invitation)
	for i := range nodes {
		fk := nodes[i].AcceptedBy
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "accepted_by" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *InvitationQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withProject != nil {
			_spec.Node.AddColumnOnce(invitation.FieldProjectID)
		}
		if _q.withInviter != nil {
			_spec.Node.AddColumnOnce(invitation.FieldInvitedBy)
		}
		if _q.withAcceptor != nil {
			_spec.Node.AddColumnOnce(invitation.FieldAcceptedBy)
		}
	}
